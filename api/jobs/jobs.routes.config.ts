...
export class JobsRoutes extends CommonRoutesConfig {
  ...
  configureRoutes() {
    this.app
      ...
      .post(
        body('noOfFiles').isNumeric().withMessage('noOfFiles must be provided and its value > 1'),
        body('noOfEntriesPerFile').isNumeric().withMessage('noOfEntriesPerFile must be provided and its value > 0'),
    ...
    this.app
      ...
      .all(JobsMiddleWare.validateJobExists)
      .post(JobsMiddleWare.validateJobIsNotInFailedState, JobsController.linearValidator);

    this.app
      .route(`/jobs/:jobId/processed-task`)
      .all(JobsMiddleWare.validateJobExists)
      .post(
        body('processedFilesPaths').isArray().isLength({ min: 1 }).withMessage('processedFilesPaths must be > 1'),
        body('generatedFilePath').isString().withMessage('generatedFilePath must be present'),
        BodyValidationMiddleware.verifyBodyFieldsErrors,
